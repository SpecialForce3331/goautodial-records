...
func parseConfig(file *os.File) Config {
	...
	if HTTP_PORT == "" || SQL_LOGIN == "" || SQL_PASSWORD == "" || SQL_HOST == "" || SQL_PORT == "" || SQL_DB == "" || SQL_LOGIN == "" || SQL_FIELD_INBOUND_list_id == "" {
		log.Fatal("Config error! One of fields are null")
	}

	return Config{HTTP_PORT, SQL_LOGIN, SQL_PASSWORD, SQL_HOST, SQL_PORT, SQL_DB, SQL_FIELD_INBOUND_list_id}
